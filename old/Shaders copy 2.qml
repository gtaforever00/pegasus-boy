...
Item {
    ...
    ShaderEffect {
        ...
        property real scanlineDownscale: 0.5
        ...
        fragmentShader: "
            ...
            void main()
            {
                ...
                vec2 ratio_scale = coord * sourceSize - 0.5;
                vec2 uv_ratio = fract(ratio_scale / scanlineDownscale);

                coord = (floor(ratio_scale) + 0.5) / sourceSize;
                coord.x = xblur;

                vec4 col = texture2D(source, coord);
                col = pow(col, vec4(inputGamma));
                vec4 col2 = texture2D(source,coord + vec2(0.0, sourceSize.x));
